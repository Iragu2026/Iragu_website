...
export default function Salwars() {
  ...
  }, [dispatch, pageParam, sortBy, priceMin, priceMax, inStock, selectedType, selectedFabric, selectedOccasion, selectedColour, selectedSizes]);
  ...
  }, [sortBy, inStock, priceMin, priceMax, selectedSizes, selectedType, selectedFabric, selectedOccasion, selectedColour, pageParam, searchParams, setSearchParams]);
  ...
  const filterContent = (
    <div className="space-y-6">
      ...
      <div>
        ...
        >
          Size
          {openSections.size ? <FiChevronUp size={16} /> : <FiChevronDown size={16} />}
        </button>
        {openSections.size && (
          <div className="mt-3 grid grid-cols-3 gap-2">
            {SIZES.map((size) => {
              ...
              return (
                <label
                  key={size}
                  className="inline-flex items-center gap-2 rounded-lg border border-black/10 bg-white px-3 py-2 text-xs font-semibold text-[#4a4a4a]"
                >
                  <input
                    type="checkbox"
                    checked={checked}
                    onChange={(e) => {
                      ...
                      setSelectedSizes((prev) => {
                        ...
                        return on
                          ? Array.from(new Set([...current, size]))
      ...
      <div>
        ...
        >
          Type
          {openSections.type ? <FiChevronUp size={16} /> : <FiChevronDown size={16} />}
        </button>
        {openSections.type && (
          <ul className="mt-3 space-y-1.5">
            {GARMENT_TYPES.map((gt) => (
              <li key={gt}>
                <button
                  type="button"
                  onClick={() => setSelectedType((prev) => (prev === gt ? "" : gt))
                  }
      ...
      <div>
        ...
        >
          Colour
          {openSections.colour ? <FiChevronUp size={16} /> : <FiChevronDown size={16} />}
        </button>
        {openSections.colour && (
          <ul className="mt-3 space-y-1.5">
            {COLOURS.map((col) => (
              <li key={col.name}>
                <button
                  type="button"
                  onClick={() => setSelectedColour((prev) => (prev === col.name ? "" : col.name))
                  }
    ...
    <div className="min-h-screen bg-[#fbf7f0]">
      ...
      <div className="container-page pt-5 pb-2">
        <nav className="flex items-center gap-2 text-[11px] uppercase tracking-[0.15em] text-[#9a9a9a]">
          ...
          <span>/</span>
          <span className="text-[#4a4a4a]">All Salwars</span>
        </nav>
      </div>
      ...
      {/* ── Active filter tags ── */}
      {(selectedSizes.length || selectedType || selectedFabric || selectedOccasion || selectedColour) && (
        <div className="container-page flex flex-wrap items-center gap-2 py-3">
          {selectedSizes.map((sz) => (
            ...
            >
              ...
              <button
                type="button"
                onClick={() => setSelectedSizes((prev) => prev.filter((x) => x !== sz))}
